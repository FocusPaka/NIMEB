...
pD <- 0.6
n.diffexp <- n.vars * pD
fraction.upregulateds <- c(0.5,0.7,0.9)
effect.sizes <- 8
reps <- 10
dispersions <- rep(0.01,n.vars)
nc <- length(fraction.upregulateds)
...
for (i in 1:reps) {
    for (j in 1:nc) {
        B_625_625 <- generateSyntheticData(dataset = paste0("B", i, j), 
                                           ...
                                           fraction.upregulated = fraction.upregulateds[j], 
...
boxplot(X3~name,data,add=T,at=14:19,cex.axis = 0.6,las=2,
        col = c(miscolores[1:6]))
#boxplot(X4~name,data,add=T,at=21:26,cex.axis = 0.6,las=2,
#        col = c(miscolores[1:6]))

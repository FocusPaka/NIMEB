...
for(i in 1:reps){
    for(j in 1:nc){
        ...
        xx_new1 <- generateDataset2(commonTags=15000, uniqueTags=c(1000,800), 
                                    ...
                                    pDifferential=0.6, foldDifference=foldDiff, pUp=0.9,
                                    libLimits=c(.9,1.2)*1e6,nreps = c(2,2))
...
data <- data.frame(matrix(NA,reps*length(methods_name),length(foldDiffs)))
data[,1] <- name
data[,2] <- auc_whole[,1]
data[,3] <- auc_whole[,2]
data[,4] <- auc_whole[,3]
